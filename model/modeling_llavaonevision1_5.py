...
class LLaVAOneVision1_5_CausalLMOutputWithPast(ModelOutput):
    ...
    attentions: Optional[Tuple[torch.FloatTensor]] = None
    rope_deltas: Optional[torch.LongTensor] = None
    anomaly_map: Optional[torch.FloatTensor] = None
...
class LLaVAOneVision1_5_Attention(nn.Module):
    ...
    def __init__(
        self, config: LLaVAOneVision1_5_TextConfig, layer_idx: Optional[int] = None
    ):
        super().__init__()
        self.config = config
...
class LLaVAOneVision1_5_DecoderLayer(nn.Module):
    def __init__(self, config: LLaVAOneVision1_5_TextConfig, layer_idx: int):
        ...
        self.self_attn = LLaVAOneVision1_5_ATTENTION_CLASSES[
            config._attn_implementation
        ](config, layer_idx)

        self.mlp = LLaVAOneVision1_5_MLP(config)
        self.input_layernorm = LLaVAOneVision1_5_RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = LLaVAOneVision1_5_RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class RiceTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.blocks = nn.ModuleList(
            [
                RiceBlock(config, config._attn_implementation)
    ...
    def forward(
        ...
        for i in range(1, num_segments + 1):
            ...
            new_hidden[write_ptr + 1 : write_ptr + 1 + seg_len] = hidden_states[
                seg_start:seg_end
            ]
            new_rotary_pos_emb[write_ptr + 1 : write_ptr + 1 + seg_len] = (
                rotary_pos_emb[seg_start:seg_end]
            )
            write_ptr += 1 + seg_len
        ...
        cu_seqlens = torch.tensor(
            new_cu, device=hidden_states.device, dtype=torch.int32
        )
...
class LLaVAOneVision1_5_TextModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config: LLaVAOneVision1_5_TextConfig):
        ...
        self.layers = nn.ModuleList(
            [
                LLaVAOneVision1_5_DecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = LLaVAOneVision1_5_RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class LLaVAOneVision1_5_Model(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config: Llavaonevision1_5Config):
        ...
        self.language_model = LLaVAOneVision1_5_TextModel._from_config(
            config.text_config
        )

        # Custom projection for segmentation path
        self.seg_projector = RicePatchMerger(
    ...
    def get_image_features(
        ...
        if return_unmerged:
            image_embeds, unmerged = self.visual(
                pixel_values, grid_thw=image_grid_thw, return_unmerged=True
            )
            return image_embeds, unmerged
        else:
    ...
    def forward(
        ...
        output = LLaVAOneVision1_5_ModelOutputWithPast(
            ...
            image_embeds=image_embeds_for_seg,
        )
...
class LLaVAOneVision1_5_ForConditionalGeneration(
...
AutoModelForCausalLM.register(
    Llavaonevision1_5Config, LLaVAOneVision1_5_ForConditionalGeneration
)
